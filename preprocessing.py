...
EPILOG = """Given an input, generates one-hot encoded input and output mappings
for a rolling window over each input.
...
class StatisticalProb:
    ...
    def calc_stats(self, words):
        ...
        cnts2 = df_letters.pivot_table(index='first',
                                       columns='second',
                                       aggfunc='count')
    ...
    def to_json(self):
        """Serialize to JSON"""
        json_dict = {
            '_first_prob': self._first_prob.to_json(),
            '_second_prob': self._second_prob.to_json(),
        }
...
class Preprocessor:
    ...
    def preprocess(self, input_filename, window, data=None, shuffle=True):
        ...
        # Loop through all names, make uppercase, skip names having a backslash
        tgts = []
        letters = set()
        for line in txt:
            txt = line.upper().replace("^", "").\
                               replace("$", "").\
                               replace(" ", "_")
            if "/" not in txt:
                tgts.append(txt)
        ...
        if shuffle:
            ...
            idx = np.random.choice(list(range(len(self._targets))), size=len(
                self._targets), replace=False)
        ...
        self.label = os.path.split(self.filename)[-1].lower().\
            replace(".txt", "")
    ...
    def _encode_in_out(self, x_in, y_in, pos):
        ...
        x_vector = np.concatenate([np.array(pos/self._max_length).reshape([
            1]), x_vector])
    ...
    def get_rnn_format(self):
        ...
        x_train_rnn = x_train_np.reshape(len(self.x_train), self.window,
                                         num_features)
        x_test_rnn = x_test_np.reshape(len(self.x_test), self.window,
                                       num_features)
