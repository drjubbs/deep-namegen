...
class TestPreprocessor(unittest.TestCase):
    ...
    def test_preprocess(self):
        ...
        self.assertEqual(str(context.exception), "Letters present in names "
                                                 "missing in encoding.")

        sample_data = ["AAAAAA", "BBBBBB", "CCCCCC"]
        with self.assertRaises(ValueError) as context:
            self.pre.preprocess(None, 3, data=sample_data)
        self.assertEqual(str(context.exception), "At least 4 samples needed "
                                                 "to encode.")
    ...
    def test_get_rnn_format(self):
        ...
        mask_end = [t == "$" for t in pp.LETTERS]
        ...
        # The 7th training element should be end of sequence character
        self.assertTrue(np.all(y_train_rnn[6] == mask_end))
        ...
        self.assertTrue(np.all(np.isclose(y_test_rnn[0], prob_caret)))

        # 4th train y is "D"
        self.assertTrue(np.all(x_test_rnn[3][0] == mask_d))

        # 2nd test y is "D"
        self.assertTrue(np.all(y_test_rnn[1] == mask_d))
    ...
    def test_create_input_output(self):
        ...
        human, test_x, test_y = self.pre._create_input_output([
            "PHILA'_DELPHIA"])

        self.assertEqual(''.join([t[-1] for t in human['input']]),
                         "^PHILA'_DELPHIA")

        self.assertEqual(''.join(list(human['target'])), "PHILA'_DELPHIA$")

        # Check position indicator in X vector
        check_x0 = np.array([t/self.pre.get_max_length() for t in range(len(
            "PHILA' DELPHIA$"))])
        self.assertTrue(all(check_x0 == test_x[:, 0]))
        self.assertEqual(test_y.shape, (len("PHILA' DELPHIA")+1,
                                        len(pp.LETTERS)))
    ...
    def test_serialization(self):
        ...
        self.assertTrue(np.all(pre_test.statistics.get_second_df().index ==
                               pre_orig.statistics.get_second_df().index))
        self.assertTrue(np.all(pre_test.statistics.get_second_df().columns ==
